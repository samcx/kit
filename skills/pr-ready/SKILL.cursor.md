...
Mark the current PR ready, optionally add explicitly requested GitHub assignees, prompt in chat to choose GitHub reviewers from a team, add reviewers with gh CLI, post to a daily Slack thread via Slack MCP tools, and create a Linear ticket via Linear MCP tools.
...
13. Post to the daily Slack thread using MCP tools (see Slack Posting below). For a batch, post one combined message, not one message per PR.
...
1. **Find the daily thread**: Use `slack_read_channel` with `channel_id` from config and `limit: 20`. Find the message whose text contains the `thread_match` pattern from config. Extract its `ts` (or `thread_ts`) as the thread timestamp.
...
Use the Linear MCP tools to create a ticket early in the workflow (step 4) so it can be linked in the PR description.
...
2. Save the returned ticket identifier (e.g. `VA-1234`) and URL for use in step 5 (linking in PR description) and the outcome summary.
